...
export class MyApp {
      ...
      if(searchParam){
        ...
        this.rootPage = Constants.AUTH_EXT_PAGE.name;
          ...
          this.userService.getUserLocal().then(userID => {
            ...
            if (userID) {
              this.userService.byId(userID).take(1).subscribe((user: User) => {
  ...
  configureTranslate(translate) {
    ...
    translate.use(this.browserLang.match(/pt|en|fr|es/) ? this.browserLang : 'pt');
  }
  ...
  private traduz(key: string) {
    return new Promise<any> ((resolve, reject) => {
      this.translate.get(key).take(1).subscribe(text => {
        resolve(text);
      });
    });
  }
  ...
  goLogout() {
    this.genericComponents.showConfirm(this.translate.instant('menu.logout'), this.translate.instant('menu.logout_desc'),
        () => {
          this.menu.close();
