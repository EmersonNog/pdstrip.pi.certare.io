...
export class AreasExamesPage {
  ...
  openFilterPage() {
    ...
    modal.onDidDismiss((data) => {
      ...
      // Atualiza as camadas com base nos novos valores
      this.toggleRotas();
  ...
  toggleRotas() {
    if (this.rotasAtivas) {
      this.mapUtil.showRodoviaLines(this.rotas, this.map);
    } else {
  ...
  filtrar(
    ...
    if (filtro.imoveis.ativo) {
      this.imoveis.length > 0 &&
        this.imoveis.forEach((imovel) => {
          if (
            filtro.imoveis.tiposSetores.length > 0 &&
            !filtro.imoveis.tiposSetores.includes(imovel.element.tipo_setor)
          ) {
            return;
          }
          if (
            filtro.imoveis.tiposOcupacoes.length > 0 &&
          ...
          if (
            filtro.imoveis.fontes.length > 0 &&
            !filtro.imoveis.fontes.includes(imovel.element.fonte)
          ) {
            return;
          }
    ...
    if (filtro.bufferEstacoes.ativo) {
      this.bufferEstacoes.length > 0 &&
        this.bufferEstacoes.forEach((buffer) => {
          this.mapUtil.addPolyline(
            buffer.polyline,
            this.map,
            buffer.cor,
    ...
    if (filtro.bufferLinhas.ativo) {
      this.bufferLinhas.length > 0 &&
        this.bufferLinhas.forEach((buffer) => {
          this.mapUtil.addPolyline(
            buffer.polyline,
            this.map,
            buffer.cor,
  ...
  private initMap(route = undefined) {
    if (!this.map) {
      ...
      this.acidentesAtivos = false;
      this.pontosSecaoAtivos = false;
    }
    ...
    this.afd
      .collection(
        Constants.PATH_DOCUMENTS_IMOVEIS,
        (ref) => ref
        // .where('ano', '==', ano)
        // .orderBy('numero')
      )
      ...
      .subscribe((_data) => {
        ...
        if (data2) {
          ...
          arrayData.forEach((element) => {
            ...
            if (
              !municipios.find(
                (municipio) => municipio.municipio === element.municipio
              ) &&
              !!element.municipio
            ) {
            ...
            if (
              !bairros.find((item) => item.bairro === element.bairro) &&
              !!element.bairro
            ) {
              bairros.push({
            ...
            if (
              !tiposOcupacoes.find((item) => item === element.tipo_ocup) &&
              !!element.tipo_ocup
            ) {
            ...
            if (
              !tiposSetores.find((item) => item === element.tipo_setor) &&
              !!element.tipo_setor
            ) {
            ...
            if (
              !fontes.find((item) => item === element.fonte) &&
              !!element.fonte
            ) {
    ...
    this.afd
      .collection(
        Constants.PATH_DOCUMENTS_BUFFER_ESTACOES,
        (ref) => ref
        // .where('ano', '==', ano)
        // .orderBy('numero')
      )
    ...
    this.afd
      .collection(
        Constants.PATH_DOCUMENTS_BUFFER_LINHAS,
        (ref) => ref
        // .where('ano', '==', ano)
        // .orderBy('numero')
      )
    ...
    this.afd
      .collection(
        Constants.PATH_DOCUMENTS_ESTACOES,
        (ref) => ref
        // .where('ano', '==', ano)
        // .orderBy('numero')
      )
    ...
    this.afd
      .collection(
        Constants.PATH_DOCUMENTS_LINHAS,
        (ref) => ref
        // .where('ano', '==', ano)
        // .orderBy('numero')
      )
  ...
  openImagensPage(event) {
    ...
    const params = {
      ...
      pathImage: rodovia,
      key: key,
      levantamentoId: "areas_de_exames",
      type: "exames",
    };

    this.modalCtrl.create("ImagensTrechoPage", params).present();
  }
}
